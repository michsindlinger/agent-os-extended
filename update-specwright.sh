...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "This script:"
            echo "- Detects existing Specwright installations"
...
if [[ "$INSTALL_TYPE" == "none" ]]; then
    echo "❌ No Specwright installation found!"
    echo ""
...
AGENT_OS_PATH="$BASE_PATH/.specwright"
...
if [[ ! -f "$AGENT_OS_PATH/estimations/config/industry-benchmarks.json" ]]; then
    ...
    curl -sSL "$REPO_URL/test-project/.specwright/estimations/config/industry-benchmarks.json" \
    ...
    echo "  💡 Check test-project/.specwright/estimations/config/industry-benchmarks.json for latest template"
fi

if [[ ! -f "$AGENT_OS_PATH/estimations/config/estimation-config.json" ]]; then
    echo "  Creating estimation-config.json..."
    curl -sSL "$REPO_URL/test-project/.specwright/estimations/config/estimation-config.json" \
    ...
    echo "  💡 Check test-project/.specwright/estimations/config/estimation-config.json for latest template"
fi

if [[ ! -f "$AGENT_OS_PATH/estimations/history/index.json" ]]; then
    echo "  Creating history/index.json..."
    curl -sSL "$REPO_URL/test-project/.specwright/estimations/history/index.json" \
...
if [[ ! -f "$AGENT_OS_PATH/estimations/README.md" ]]; then
    ...
    curl -sSL "$REPO_URL/test-project/.specwright/estimations/README.md" \
         -o "$AGENT_OS_PATH/estimations/README.md"
else
    # Always update README as it's documentation, not configuration
    update_file "$REPO_URL/test-project/.specwright/estimations/README.md" \
...
echo "  • .specwright/instructions/ - Core workflow instructions"
echo "  • .specwright/standards/    - Development standards"
echo "  • .specwright/specs/        - Feature specifications"
echo "  • .specwright/docs/         - Feature documentation"
echo "  • .specwright/bugs/         - Bug tracking and resolution"
echo "  • .specwright/estimations/  - Effort estimation system"
