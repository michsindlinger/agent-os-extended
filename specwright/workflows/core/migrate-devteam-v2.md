...
- Skills are stored in `specwright/skills/` (not `.claude/skills/`)
...
3. Move/convert skills from `.claude/skills/` to `specwright/skills/`
...
<pre_flight>
  ...
  IF specwright/team/skill-index.md EXISTS:
...
<user_confirmation>
  ...
  3. Move skills to specwright/skills/ (flat structure)
  4. Generate specwright/team/skill-index.md
  ...
  Files to be created:
  ─────────────────────
  - specwright/skills/*.md (migrated skills)
  - specwright/team/skill-index.md
...
Move skills from `.claude/skills/` to `specwright/skills/`.
...
<skill_migration>
  ...
  2. FOR EACH skill in .claude/skills/:
     ...
     <determine_target>
       ...
       TARGET = specwright/skills/[prefix][skill-name].md
     </determine_target>
...
**Output:** Skills in `specwright/skills/` with Quick Reference sections
...
<skill_index_generation>
  ...
  2. LOAD template:
     TRY: specwright/templates/docs/skill-index-template.md
     FALLBACK: ~/.specwright/templates/docs/skill-index-template.md

  3. POPULATE template:
     - [PROJECT_NAME] → Extract from product-brief.md or use directory name
     - [CURRENT_DATE] → Today's date

  4. FOR EACH skill in specwright/skills/:
     READ: Skill file
     EXTRACT: Skill name, trigger keywords (from "When to Activate")
     ADD: To appropriate category table in skill-index.md

  5. WRITE: specwright/team/skill-index.md

  6. LOG: "✓ Generated: specwright/team/skill-index.md"
</skill_index_generation>

**Output:** `specwright/team/skill-index.md`
...
<verification_checks>
  ...
  ```bash
  ls specwright/skills/*.md | wc -l
  # Should match expected count
  ```

  CHECK 3: Skill-index exists
  ```bash
  test -f specwright/team/skill-index.md && echo "✓ skill-index.md exists"
  ```
...
<summary>
  ...
  ✓ [N] skills migrated to specwright/skills/
  ✓ skill-index.md generated at specwright/team/skill-index.md
  ✓ Old skills backed up to .claude/skills-v1-backup/

  New Architecture:
  ─────────────────────
  • Skills are in specwright/skills/ (not assigned to agents)
  ...
  Next Steps:
  ─────────────────────
  1. Review specwright/team/skill-index.md
...
<rollback>
  ...
  3. Remove generated files:
     ...
     rm -f specwright/team/skill-index.md
     ```
