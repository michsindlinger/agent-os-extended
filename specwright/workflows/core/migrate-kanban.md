...
<mandatory_actions>
  ...
  2. AUTO-DETECTION:
     ...
     SPECS_TO_MIGRATE=$(for spec in specwright/specs/*/; do
     ...
     if [[ -f "specwright/backlog/story-index.md" ]] && [[ ! -f "specwright/backlog/backlog.json" ]]; then
...
<mandatory_actions>
  1. CHECK: Does specwright/backlog/ exist?
     IF NOT: SKIP backlog migration

  2. CREATE directory structure:
     ```bash
     mkdir -p specwright/backlog/stories
  ...
  3. SCAN for existing story files:
     ...
     BUG_FILES=$(ls specwright/backlog/bug-*.md 2>/dev/null)
     TODO_FILES=$(ls specwright/backlog/user-story-*.md 2>/dev/null)
     ```
  ...
  4. FOR EACH story file found:
     ...
     ```bash
     mv specwright/backlog/bug-*.md specwright/backlog/stories/ 2>/dev/null
     mv specwright/backlog/user-story-*.md specwright/backlog/stories/ 2>/dev/null
     ```

  5. BUILD backlog.json:

     LOAD template: specwright/templates/json/backlog-template.json
  ...
  6. WRITE: specwright/backlog/backlog.json

  7. BACKUP old file:
     ```bash
     mv specwright/backlog/story-index.md specwright/backlog/story-index.md.bak
     ```

  8. VERIFY:
     ```bash
     cat specwright/backlog/backlog.json | python3 -m json.tool > /dev/null && echo "Valid JSON"
     ```
  ...
  9. REPORT:
     ...
     **Files:**
     - Created: specwright/backlog/backlog.json
...
<mandatory_actions>
  1. READ: specwright/specs/[SELECTED_SPEC]/kanban-board.md
  ...
  5. WRITE: specwright/specs/[SELECTED_SPEC]/kanban.json

  6. BACKUP old file:
     ```bash
     mv specwright/specs/[SELECTED_SPEC]/kanban-board.md \
        specwright/specs/[SELECTED_SPEC]/kanban-board.md.bak
     ```

  7. VERIFY:
     ```bash
     cat specwright/specs/[SELECTED_SPEC]/kanban.json | python3 -m json.tool > /dev/null
     ```
...
<mandatory_actions>
  ...
  3. FINAL REPORT:
     ...
     **Backup Files Created:**
     - specwright/backlog/story-index.md.bak
     - specwright/specs/[spec-1]/kanban-board.md.bak
     - ...
...
```bash
# Restore backlog
mv specwright/backlog/story-index.md.bak specwright/backlog/story-index.md
rm specwright/backlog/backlog.json

# Restore spec
mv specwright/specs/[spec]/kanban-board.md.bak specwright/specs/[spec]/kanban-board.md
rm specwright/specs/[spec]/kanban.json
```
