---
description: Rules to initiate execution of a set of tasks using Specwright
globs:
alwaysApply: false
version: 2.2
encoding: UTF-8
...
**Supports two modes:**
- **Spec Mode**: Full feature execution from `specwright/specs/[spec-name]/`
- **Backlog Mode**: Quick task execution from `specwright/backlog/`
...
<mode_detection>
  ...
  <user_mode_selection>
    ...
    SPEC_KANBANS=$(ls specwright/specs/*/kanban-board.md 2>/dev/null | head -5)

    # Check for active backlog kanban (today)
    TODAY=$(date +%Y-%m-%d)
    BACKLOG_KANBAN=$(ls specwright/backlog/kanban-${TODAY}.md 2>/dev/null)

    # Check for pending backlog stories (user-story-* AND bug-*)
    BACKLOG_STORIES=$(ls specwright/backlog/user-story-*.md specwright/backlog/bug-*.md 2>/dev/null | wc -l)
    ```
    ...
    ELSE IF backlog has stories AND specs exist:
      ...
      1. Execute Backlog ([N] quick tasks)
         → Run tasks from specwright/backlog/
...
<backlog_mode_detection>
  ...
  2. CHECK: Does today's kanban exist?
     ```bash
     ls specwright/backlog/kanban-${TODAY}.md 2>/dev/null
     ```
...
<spec_mode_detection>
  ...
  1. CHECK: Does kanban-board.md exist for target/any spec?
     ```bash
     ls specwright/specs/*/kanban-board.md 2>/dev/null | head -1
     ```
...
- No kanban-[TODAY].md exists in specwright/backlog/
...
<step name="collect_ready_stories">
  ...
  ```bash
  ls specwright/backlog/user-story-*.md specwright/backlog/bug-*.md 2>/dev/null
  ```
...
<step name="create_daily_kanban" subagent="file-creator">
  ...
  Output: specwright/backlog/kanban-{TODAY}.md
...
<phase_complete>
  ...
  **Kanban:** specwright/backlog/kanban-{TODAY}.md
...
<step name="extract_skill_patterns_backlog">
  ...
  <skill_extraction_process>
    1. READ: Story file (specwright/backlog/{STORY_FILE})
    ...
    3. IF "Relevante Skills" NOT found:
       ...
       READ: specwright/team/skill-index.md (if exists)
...
<step name="execute_story" subagent="dev-team">
  ...
  **Story File:** specwright/backlog/{STORY_FILE}
...
<step name="update_story_index">
  READ: specwright/backlog/story-index.md
...
<step name="archive_completed_stories">
  ...
  ```bash
  mkdir -p specwright/backlog/archive/{TODAY}
  # Archive both user-story-* and bug-* files
  mv specwright/backlog/user-story-*.md specwright/backlog/bug-*.md specwright/backlog/archive/{TODAY}/ 2>/dev/null
  ```
...
<step name="create_kanban_board" subagent="file-creator">
  ...
  PROMPT: "Create kanban board for spec: specwright/specs/{SELECTED_SPEC}/

  Source: Parse story files in stories/ directory (NOT user-stories.md)
  Output: specwright/specs/{SELECTED_SPEC}/kanban-board.md
  Template: specwright/templates/docs/kanban-board-template.md (global) or specwright/templates/docs/kanban-board-template.md (project)

  CRITICAL STEPS:
  1. LIST all story files in specwright/specs/{SELECTED_SPEC}/stories/
...
<phase_complete>
  ...
  **Created:**
  - Kanban Board: specwright/specs/{SELECTED_SPEC}/kanban-board.md
...
<step name="git_worktree_creation" subagent="git-workflow">
  ...
  **CRITICAL: Use Git Worktree for Parallel Execution**
  ...
  3. Create git worktree:
     ...
     WORKTREE_BASE='specwright/worktrees'
  ...
  5. Return these values for Resume Context:
     - WORKTREE_PATH: specwright/worktrees/$WORKTREE_NAME
  ...
  **Examples:**
  - Spec: 2026-01-13-multi-delete-projects
    → Worktree: specwright/worktrees/multi-delete-projects
    → Branch: multi-delete-projects

  - Spec: 2026-01-12-bugfix-login-error
    → Worktree: specwright/worktrees/bugfix-login-error
    → Branch: bugfix/login-error

  - Spec: 2026-01-14-user-authentication
    → Worktree: specwright/worktrees/user-authentication
...
<step name="extract_skill_patterns">
  ...
  <skill_extraction_process>
    1. READ: Story file (specwright/specs/{SELECTED_SPEC}/stories/story-XXX-[slug].md)
    ...
    2. FIND: "### Relevante Skills" section
       ...
       | Skill | Pfad | Grund |
       |-------|------|-------|
       | Logic Implementing | specwright/skills/backend-logic-implementing.md | Service Object |
       | Test Engineering | specwright/skills/backend-test-engineering.md | Unit Tests |

    3. IF "Relevante Skills" section NOT found OR empty:
       FALLBACK: Use skill-index.md to auto-select based on story type (WER field)
       READ: specwright/team/skill-index.md
...
<step name="execute_story" subagent="dev-team">
  ...
  **Story Details:**
  [Read story file from specwright/specs/{SELECTED_SPEC}/stories/story-XXX-[slug].md]
  ...
  **Project Context:**
  - Tech Stack: specwright/product/tech-stack.md (key info only)
  ...
  **File Organization (CRITICAL):**
  ...
  ✅ Handovers: specwright/specs/{SELECTED_SPEC}/handover-docs/
  "
...
<step name="architect_review" subagent="dev-team__architect">
  ...
  Context:
  - Story: specwright/specs/{SELECTED_SPEC}/stories/story-XXX-[slug].md
...
<step name="handle_integration_failures">
  ...
  ELSE (some tests FAILED):
    ...
    - File: specwright/specs/{SELECTED_SPEC}/stories/story-999-integration-fix.md
...
<phase_complete>
  ...
  - View: specwright/specs/{SELECTED_SPEC}/kanban-board.md
...
<resume_context_schema>
  ...
  **Template Location**:
  - Global: ~/.specwright/templates/docs/kanban-board-template.md
  - Project: specwright/templates/docs/kanban-board-template.md
  ...
  | **Spec Folder** | Full path | specwright/specs/2026-01-13-multi-delete-projects |
  | **Worktree Path** | Path to git worktree for parallel execution | specwright/worktrees/multi-delete-projects or Pending |
