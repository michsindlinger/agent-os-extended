...
Create a structured bug specification that integrates with the standard feature workflow. Bug-Specs are created in `specwright/specs/` and executed via `execute-tasks`, ensuring consistent quality gates, git management, and agent delegation.
...
<delegation>
  ...
  **Your Tasks:**
  ...
  2. Create Bug Spec Structure:
     ...
     **spec.md** (specwright/specs/[SPEC_FOLDER]/spec.md):
     ```markdown
     ...
     **user-stories.md** (specwright/specs/[SPEC_FOLDER]/user-stories.md):
     ```markdown
...
<delegation>
  ...
  **Spec Location:** specwright/specs/[SPEC_FOLDER]/
...
<validation_process>
  READ: specwright/specs/[SPEC_FOLDER]/user-stories.md
  ...
  ELSE (stories flagged):
    ...
    <user_choice_handling>
      IF choice = "Review and manually edit":
        INFORM: "Please edit: specwright/specs/[SPEC_FOLDER]/user-stories.md"
...
```
specwright/specs/YYYY-MM-DD-bugfix-[bug-name]/
