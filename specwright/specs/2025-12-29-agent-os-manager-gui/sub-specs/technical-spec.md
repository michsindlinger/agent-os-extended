# Technical Specification - Specwright Manager GUI
...
function resolveSkill(name: string): SkillLocation {
  ...
  const globalBase = process.env.HOME + '/.specwright/skills';
  const found = glob.sync(`${globalBase}/**/${name}.md`);
  if (found.length > 0) {
    return { source: 'global', path: found[0] };
  }

  // 3. Check specwright repo (if running from there)
  const repoPath = path.join(__dirname, '../../specwright/skills/**', `${name}.md`);
