...
Scenario: Agent arbeitet nicht im Worktree-Verzeichnis
  ...
  And Worktree Path ist "specwright/worktrees/my-feature"
...
Scenario: Agent arbeitet im korrekten Worktree
  Given eine Spec hat Git Strategy "worktree" im Resume Context
  And Worktree Path ist "specwright/worktrees/my-feature"
  And das aktuelle Working Directory ist "specwright/worktrees/my-feature"
  When /execute-tasks aufgerufen wird
  Then wird KEINE Warnung angezeigt
  And die Execution wird normal fortgesetzt
```
...
**WO:**
- `specwright/workflows/core/execute-tasks/entry-point.md` - CWD Check hinzufügen
- `specwright/workflows/core/execute-tasks/shared/resume-context.md` - Git Strategy Feld dokumentieren
...
grep -q "Git Strategy" specwright/workflows/core/execute-tasks/entry-point.md && echo "PASS: Git Strategy check" || echo "FAIL: Git Strategy missing"

# Entry-Point enthält Worktree Warning
grep -q "worktree" specwright/workflows/core/execute-tasks/entry-point.md && echo "PASS: Worktree handling" || echo "FAIL: Worktree handling missing"

# Resume Context dokumentiert Git Strategy
grep -q "Git Strategy" specwright/workflows/core/execute-tasks/shared/resume-context.md && echo "PASS: Resume Context updated" || echo "FAIL: Resume Context missing Git Strategy"
```
