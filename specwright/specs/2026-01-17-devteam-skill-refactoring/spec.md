...
│                                                                         │
│  Generiert:                                                             │
│  ├── specwright/skills/backend-logic-implementing.md (projekt-spezifisch) │
│  ├── specwright/skills/backend-persistence-adapter.md                     │
│  ├── specwright/skills/frontend-ui-component.md                           │
│  └── ...                                                                │
│                                                                         │
│  Generiert auch:                                                        │
│  └── specwright/team/skill-index.md (Übersicht aller Skills + Trigger)    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────┐
...
**Betroffene Dateien:**
- `specwright/templates/agents/dev-team/backend-developer-template.md`
- `specwright/templates/agents/dev-team/frontend-developer-template.md`
- `specwright/templates/agents/dev-team/devops-specialist-template.md`
- `specwright/templates/agents/dev-team/qa-specialist-template.md`
- `specwright/templates/agents/dev-team/po-template.md`
- `specwright/templates/agents/dev-team/documenter-template.md`
- `specwright/templates/agents/dev-team/architect-template.md`
...
Siehe: specwright/team/skill-index.md
```
...
Skills werden weiterhin generiert, aber in `specwright/skills/` (nicht `.claude/skills/`):
...
Nach der Skill-Generierung wird `specwright/team/skill-index.md` erstellt:
...
|-------|------|------------------|--------------|
| logic-implementing | specwright/skills/backend-logic-implementing.md | service, business logic, domain, use case, validation, workflow | Service Objects, Domain Logic, Validation |
| persistence-adapter | specwright/skills/backend-persistence-adapter.md | database, query, migration, model, repository, ActiveRecord, SQL | Database Access, Queries, Migrations |
| integration-adapter | specwright/skills/backend-integration-adapter.md | API call, external service, webhook, HTTP client, integration | External API Integration |
| test-engineering | specwright/skills/backend-test-engineering.md | test, spec, RSpec, unit test, fixture, mock, factory | Backend Testing Patterns |

## Frontend Skills

| Skill | Pfad | Trigger-Keywords | Beschreibung |
|-------|------|------------------|--------------|
| ui-component | specwright/skills/frontend-ui-component.md | component, view, layout, render, JSX, template | Component Architecture |
| state-management | specwright/skills/frontend-state-management.md | state, store, context, reducer, hook, zustand | State Management Patterns |
| api-bridge | specwright/skills/frontend-api-bridge.md | fetch, API client, axios, data fetching, SWR | API Integration |
| interaction-design | specwright/skills/frontend-interaction-design.md | form, input, validation, UX, user interaction | User Interaction Patterns |

## DevOps Skills

| Skill | Pfad | Trigger-Keywords | Beschreibung |
|-------|------|------------------|--------------|
| infrastructure | specwright/skills/devops-infrastructure.md | deploy, server, Docker, Kubernetes, infrastructure | Infrastructure Setup |
| pipeline | specwright/skills/devops-pipeline.md | CI/CD, GitHub Actions, pipeline, automation | CI/CD Pipelines |

## QA Skills

| Skill | Pfad | Trigger-Keywords | Beschreibung |
|-------|------|------------------|--------------|
| test-strategy | specwright/skills/qa-test-strategy.md | test plan, coverage, quality gates | Test Strategy |
| test-automation | specwright/skills/qa-test-automation.md | E2E, Playwright, integration test, automation | Test Automation |

## Architect Skills

| Skill | Pfad | Trigger-Keywords | Beschreibung |
|-------|------|------------------|--------------|
| pattern-enforcement | specwright/skills/architect-pattern-enforcement.md | architecture, pattern, structure, design | Architecture Patterns |
| api-design | specwright/skills/architect-api-design.md | API design, endpoint, REST, GraphQL | API Design |
...
```markdown
LOAD: specwright/team/skill-index.md
...
<skill_reading>
  ...
  3. FOR EACH selected skill:
     READ: specwright/skills/[skill-name].md
...
- `specwright/templates/skills/dev-team/backend/*.md`
- `specwright/templates/skills/dev-team/frontend/*.md`
- `specwright/templates/skills/dev-team/devops/*.md`
- `specwright/templates/skills/dev-team/qa/*.md`
- `specwright/templates/skills/dev-team/architect/*.md`
...
```bash
#!/bin/bash
# Migration von DevTeam v1 (Skills in .claude/skills/) zu v2 (Skills in specwright/skills/)
...
if [[ ! -d ".claude/skills" ]] && [[ -f "specwright/team/skill-index.md" ]]; then
...
# 2. Skills von .claude/skills/ nach specwright/skills/ migrieren
echo "2. Migrating skills..."
mkdir -p specwright/skills
...
if [[ -d ".claude/skills" ]]; then
  ...
  for skill_dir in .claude/skills/*/; do
    if [[ -f "${skill_dir}SKILL.md" ]]; then
      ...
      cp "${skill_dir}SKILL.md" "specwright/skills/${clean_name}.md"
...
mkdir -p specwright/team

cat > specwright/team/skill-index.md << 'SKILLINDEX'
# Skill Index
...
for skill in specwright/skills/*.md; do
  if [[ -f "$skill" ]]; then
    skill_name=$(basename "$skill" .md)
    echo "| $skill_name | $skill | TODO: Add description |" >> specwright/team/skill-index.md
  fi
done

cat >> specwright/team/skill-index.md << 'FOOTER'
...
echo "   ✓ Created: specwright/team/skill-index.md"
...
echo ""
echo "Changes:"
echo "  • Skills moved: .claude/skills/ → specwright/skills/"
echo "  • Agent backups: .claude/agents/dev-team.v1-backup/"
echo "  • Old skills archived: .claude/skills.v1-archive/"
echo "  • Skill index created: specwright/team/skill-index.md"
echo ""
echo "Next steps:"
echo "  1. Review specwright/team/skill-index.md"
...
**What changes:**
- Skills move from `.claude/skills/` to `specwright/skills/`
...
Refer to: @specwright/workflows/core/migrate-devteam-v2.md
```
...
**7.2 build-development-team generiert Skills in specwright/skills/**

- Neuer Output-Pfad: `specwright/skills/` statt `.claude/skills/`
- Generiert `specwright/team/skill-index.md`
...
1. **build-development-team** generiert Skills in `specwright/skills/`
...
Skill-Index: specwright/team/skill-index.md
```
...
│  2. Liest nur die angegebenen Skills aus specwright/skills/       │
...
### Framework (specwright)

| Datei | Änderung |
|-------|----------|
| `specwright/templates/agents/dev-team/*.md` | `skills:` entfernen, Hinweis hinzufügen |
| `specwright/workflows/core/build-development-team.md` | Output → `specwright/skills/`, skill-index.md generieren |
| `specwright/workflows/core/create-spec.md` | Architect liest skill-index.md, trägt Skills in Story ein |
| `specwright/workflows/core/execute-tasks.md` | Skill-Reading + Extraction vor Delegation |
| `specwright/templates/docs/story-template.md` | "Relevante Skills" Sektion hinzufügen |
| `specwright/templates/skills/dev-team/**/*.md` | Quick Reference Sektion hinzufügen |
| `.claude/commands/specwright/migrate-devteam-v2.md` | NEU |
| `specwright/workflows/core/migrate-devteam-v2.md` | NEU |
...
| `specwright/skills/*.md` | NEU (migrierte Skills) |
| `specwright/team/skill-index.md` | NEU |
...
- [x] build-development-team generiert Skills in `specwright/skills/`
- [x] build-development-team generiert `specwright/team/skill-index.md`
...
2. **Skills werden weiterhin generiert** - In `specwright/skills/` (projekt-spezifisch)
