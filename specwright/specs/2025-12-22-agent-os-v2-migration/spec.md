...
> Spec: Specwright - Version 2.0 Migration
> Created: 2025-12-22

## Overview

Migrate Specwright to incorporate Version 2.0 architectural improvements while maintaining our project-level installation philosophy and enterprise extensions. This migration will be executed in two phases: Phase I focuses on structural alignment with v2.0 conventions, and Phase II introduces advanced features like profile system, Claude Skills-based standards injection, enhanced research workflows, and verification systems.

## User Stories

### Story 1: Seamless Migration for Existing Projects

As a developer using Specwright in multiple projects, I want to update all my existing projects to the new v2.0 structure with a single command, so that I can benefit from the improvements without manual work in each project.

When I run the update script, it automatically detects my current Specwright installation, backs up existing configuration, migrates directory structures, updates command locations, and preserves all custom settings and enterprise extensions. The migration is non-destructive and can be rolled back if needed.
...
1. **Phase I: Structural Migration** - Rename directories (`.specwright` → `specwright`, `instructions/` → `workflows/`), isolate commands (`.claude/commands/specwright/`), update all setup scripts, and create migration scripts for existing projects
...
- Central installation in `~/specwright` (we maintain project-level installation philosophy)
...
1. Existing Specwright projects can be migrated to v2.0 structure using `update-to-v2.sh` script with automatic backup and rollback capability
2. New projects installed with updated setup scripts automatically use v2.0 structure (`specwright/`, `workflows/`, isolated commands)
