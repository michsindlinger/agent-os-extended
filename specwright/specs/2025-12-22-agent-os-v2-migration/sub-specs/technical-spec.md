...
This is the technical specification for the spec detailed in @specwright/specs/2025-12-22-specwright-v2-migration/spec.md
...
.claude/
├── commands/
│   └── specwright/          # NEW: Isolated namespace
│       ├── create-spec.md
│       ├── execute-tasks.md
│       └── ...
├── agents/
└── skills/                # NEW: Skills linked to specwright/skills/
```
...
**Functionality:**
1. **Pre-flight Check**
   - Detect existing Specwright installation
   - Verify version (check for `.specwright/` existence)
   - Check for uncommitted git changes (warn user)

2. **Backup Creation**
   - Create timestamped backup: `.specwright.backup-YYYY-MM-DD-HHMMSS/`
   - Copy entire `.specwright/` directory
   - Copy `.claude/commands/` directory
   - Store backup manifest with file list

3. **Directory Migration**
   - Rename `.specwright/` → `specwright/`
   - Rename `specwright/instructions/` → `specwright/workflows/`
   - Create `specwright/profiles/` directory
   - Create `specwright/skills/` directory

4. **Command Isolation**
   - Create `.claude/commands/specwright/` directory
   - Move all Specwright commands to new location
   - Update command references if needed

5. **Reference Updates**
   - Update all `@.specwright/` references to `@specwright/`
...
- `update-specwright.sh` - Update existing installations

**Changes:**
- Install to `specwright/` instead of `.specwright/`
- Install workflows to `specwright/workflows/`
- Install commands to `.claude/commands/specwright/`
- Install initial profiles to `specwright/profiles/`
...
```yaml
# specwright/profiles/java-spring-boot.md
---
...
**Profile Selection:**
- Configured in `specwright/config.yml` per project
...
**Skills Structure:**
```
.claude/skills/                  # Symlinked to specwright/skills/
...
Create new workflow: `specwright/workflows/research/analyze-codebase-patterns.md`
...
1. **Spec Verification** - `specwright/workflows/verify-spec.md`
   - Business requirements completeness
   - Technical specification completeness
   - Security considerations
   - Testing strategy defined

2. **Implementation Verification** - `specwright/workflows/verify-implementation.md`
...
3. **Visual Verification** - `specwright/workflows/verify-visual.md`
...
- Migration is one-way (`.specwright/` → `specwright/`)
