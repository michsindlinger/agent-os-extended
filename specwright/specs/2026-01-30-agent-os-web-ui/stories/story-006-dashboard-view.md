...
Scenario: Alle Specs des Projekts werden angezeigt
  Given das Projekt "specwright" ist ausgewählt
  And es gibt 3 Specs im specwright/specs/ Ordner
...
Scenario: Projekt ohne Specs
  Given das ausgewählte Projekt hat keinen specwright/specs/ Ordner
  When ich den Dashboard-View öffne
...
- [ ] FILE_EXISTS: specwright-ui/ui/src/views/dashboard-view.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/spec-card.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/kanban-board.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/story-card.ts
- [ ] FILE_EXISTS: specwright-ui/src/server/specs-reader.ts

### Inhalt-Prüfungen

- [ ] CONTAINS: dashboard-view.ts enthält "@customElement"
- [ ] CONTAINS: specs-reader.ts enthält "kanban-board.md"
- [ ] CONTAINS: kanban-board.ts enthält "Backlog"
...
**WAS:**
- Specs Reader (parst .specwright/specs/ Verzeichnis)
...
```bash
# Verify files exist
test -f specwright-ui/src/server/specs-reader.ts && echo "OK: specs-reader.ts exists"
test -f specwright-ui/ui/src/views/dashboard-view.ts && echo "OK: dashboard-view.ts exists"
test -f specwright-ui/ui/src/components/spec-card.ts && echo "OK: spec-card.ts exists"
test -f specwright-ui/ui/src/components/kanban-board.ts && echo "OK: kanban-board.ts exists"
test -f specwright-ui/ui/src/components/story-card.ts && echo "OK: story-card.ts exists"

# Verify kanban support
grep -q "kanban-board.md" specwright-ui/src/server/specs-reader.ts && echo "OK: Reads kanban file"
grep -q "Backlog" specwright-ui/ui/src/components/kanban-board.ts && echo "OK: Has Backlog column"

# Verify Lit components
grep -q "@customElement" specwright-ui/ui/src/views/dashboard-view.ts && echo "OK: Lit component"
