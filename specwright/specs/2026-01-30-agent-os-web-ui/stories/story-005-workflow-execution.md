...
Feature: Workflow Execution mit Progress
  Als Benutzer
  möchte ich Specwright Workflows über die UI starten können,
...
- [ ] FILE_EXISTS: specwright-ui/ui/src/views/workflow-view.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/workflow-card.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/workflow-progress.ts
- [ ] FILE_EXISTS: specwright-ui/src/server/workflow-executor.ts

### Inhalt-Prüfungen

- [ ] CONTAINS: workflow-view.ts enthält "cancel"
- [ ] CONTAINS: workflow-executor.ts enthält "query" (Agent SDK)
- [ ] CONTAINS: workflow-progress.ts enthält "elapsed"
...
**WIE:**
- Liest .claude/commands/specwright/*.md für Workflow-Liste
...
```bash
# Verify files exist
test -f specwright-ui/src/server/workflow-executor.ts && echo "OK: workflow-executor.ts exists"
test -f specwright-ui/ui/src/views/workflow-view.ts && echo "OK: workflow-view.ts exists"
test -f specwright-ui/ui/src/components/workflow-card.ts && echo "OK: workflow-card.ts exists"
test -f specwright-ui/ui/src/components/workflow-progress.ts && echo "OK: workflow-progress.ts exists"

# Verify cancel support
grep -q "cancel" specwright-ui/ui/src/views/workflow-view.ts && echo "OK: Cancel support"

# Verify elapsed timer
grep -q "elapsed" specwright-ui/ui/src/components/workflow-progress.ts && echo "OK: Elapsed timer"

# Verify Agent SDK usage
grep -q "query" specwright-ui/src/server/workflow-executor.ts && echo "OK: Uses Agent SDK"
