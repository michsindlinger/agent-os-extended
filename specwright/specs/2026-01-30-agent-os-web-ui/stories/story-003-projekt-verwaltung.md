...
Scenario: Aktuelles Projekt ist sichtbar
  Given das Projekt "specwright" ist ausgewählt
  Then sehe ich "specwright" prominent im Header
  And der Pfad "/Users/.../specwright" ist sichtbar
```
...
- [ ] FILE_EXISTS: specwright-ui/config.json
- [ ] FILE_EXISTS: specwright-ui/src/server/projects.ts
- [ ] FILE_EXISTS: specwright-ui/ui/src/components/project-selector.ts

### Inhalt-Prüfungen

- [ ] CONTAINS: config.json enthält "projects"
- [ ] CONTAINS: project-selector.ts enthält "@customElement"

### Funktions-Prüfungen

- [ ] BUILD_PASS: cd specwright-ui && npm run build
- [ ] TEST_PASS: cd specwright-ui && npm test -- --grep "project"
...
```bash
# Verify files exist
test -f specwright-ui/config.json && echo "OK: config.json exists"
test -f specwright-ui/src/server/projects.ts && echo "OK: projects.ts exists"
test -f specwright-ui/ui/src/gateway.ts && echo "OK: gateway.ts exists"
test -f specwright-ui/ui/src/components/project-selector.ts && echo "OK: project-selector.ts exists"

# Verify config structure
grep -q "projects" specwright-ui/config.json && echo "OK: projects array in config"

# Verify component registration
grep -q "@customElement" specwright-ui/ui/src/components/project-selector.ts && echo "OK: Lit component"
