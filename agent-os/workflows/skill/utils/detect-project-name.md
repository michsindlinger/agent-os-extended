...
1. Agent OS config (agent-os/config.yml)
...
### Step 1: Check Agent OS Config

Highest priority source - project name from Agent OS configuration.

<config_detection>
  IF agent-os/config.yml exists:
    READ: agent-os/config.yml file
    PARSE: YAML content
...
<confidence>
  CONFIDENCE: HIGH
  REASON: Explicitly configured by user in Agent OS
...
<output>
  RETURN:
    ...
    source: "package.json" | "agent-os-config" | etc.
...
<save_to_config>
  IF project name confirmed AND agent-os/config.yml exists:
    ASK: "Save this name to agent-os/config.yml for future use?"
    IF yes:
      UPDATE: agent-os/config.yml
...
<final_output>
  {
    ...
    confidence: "high",
    detection_path: [
      {step: "agent-os-config", result: "not_found"},
...
<example>
  ...
  PROCESS:
    1. Check agent-os/config.yml → Not found
