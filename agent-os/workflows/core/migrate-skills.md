...
<instructions>
  ...
  1. READ: agent-os/product/tech-stack.md (if exists)
     EXTRACT: Framework, language, version

  2. DETECT: Project name
     - Try: agent-os/config.yml (project.name)
     - Try: package.json (name field)
     - Try: directory name

  3. READ: @agent-os/workflows/skill/utils/globs-mapping.md
...
- `@agent-os/workflows/core/build-development-team.md` - Creates new skills with frontmatter
- `@agent-os/workflows/core/toggle-skill-activation.md` - Modify skill activation modes
- `@agent-os/workflows/skill/utils/globs-mapping.md` - Framework-specific globs
