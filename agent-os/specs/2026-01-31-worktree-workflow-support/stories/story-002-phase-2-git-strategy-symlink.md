...
Scenario: Worktree-Strategie erstellt Verzeichnis mit Symlink
  ...
  Then wird ein Worktree erstellt in "agent-os/worktrees/my-feature/"
  And ein Feature Branch "feature/my-feature" wird im Worktree erstellt
  And ein Symlink wird erstellt:
    "agent-os/worktrees/my-feature/agent-os/specs/2026-01-31-my-feature" → "../../../../agent-os/specs/2026-01-31-my-feature"
  And der Resume Context wird aktualisiert mit:
    - Git Strategy: worktree
    - Worktree Path: agent-os/worktrees/my-feature
```
...
Scenario: User erhält Anweisung für Worktree-Start
  Given ein Worktree wurde erstellt in "agent-os/worktrees/my-feature"
...
Scenario: Symlink ermöglicht Spec-Zugriff im Worktree
  ...
  Then kann er "agent-os/specs/[spec-name]/" lesen
...
IF gitStrategy == "worktree":
  1. Worktree erstellen: git worktree add agent-os/worktrees/[name] -b feature/[name]
  2. Verzeichnisstruktur erstellen: mkdir -p agent-os/worktrees/[name]/agent-os/specs
  3. Symlink erstellen: ln -s ../../../../agent-os/specs/[spec-name] agent-os/worktrees/[name]/agent-os/specs/[spec-name]
  4. Resume Context: Git Strategy = worktree, Worktree Path = agent-os/worktrees/[name]
...
- Vom Worktree `agent-os/worktrees/[name]/agent-os/specs/[spec]`
- Zum Original `agent-os/specs/[spec]`
- Relativer Pfad: `../../../../agent-os/specs/[spec]`

**WO:**
- `agent-os/workflows/core/execute-tasks/spec-phase-2.md` - Hauptänderungen
- `agent-os/workflows/core/execute-tasks/shared/resume-context.md` - Schema Update
...
grep -q "gitStrategy" agent-os/workflows/core/execute-tasks/spec-phase-2.md && echo "PASS: gitStrategy handling" || echo "FAIL: gitStrategy missing"

# Phase-2 enthält Symlink-Erstellung
grep -q "ln -s" agent-os/workflows/core/execute-tasks/spec-phase-2.md && echo "PASS: Symlink creation" || echo "FAIL: Symlink missing"

# Phase-2 enthält Branch-Alternative
grep -q "branch" agent-os/workflows/core/execute-tasks/spec-phase-2.md && echo "PASS: Branch alternative" || echo "FAIL: Branch handling missing"

# Phase-2 enthält User-Anweisung
grep -qE "(cd.*worktree|claude)" agent-os/workflows/core/execute-tasks/spec-phase-2.md && echo "PASS: User instruction" || echo "FAIL: User instruction missing"
```
