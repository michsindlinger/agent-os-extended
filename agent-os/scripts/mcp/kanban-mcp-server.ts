...
/**
 * Kanban MCP Server for Agent OS
 *
...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Add an item to the global backlog. Creates item file in agent-os/backlog/items/ and updates backlog index. Used by /add-todo and story-998 (code review findings).',
    inputSchema: {
      type: 'object',
      properties: {
        itemType: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    // Resolve paths from working directory
    const cwd = process.cwd();
    const specId = (args as { specId?: string }).specId;
    const specPath = specId ? join(cwd, 'agent-os', 'specs', specId) : cwd;
...
async function handleBacklogAddItem(
  ...
  const backlogDir = join(projectPath, 'agent-os', 'backlog');
