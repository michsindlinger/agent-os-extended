...
mkdir -p agent-os/scripts && \
curl -fsSL https://raw.githubusercontent.com/michsindlinger/agent-os-extended/main/agent-os/scripts/auto-execute.sh \
  -o agent-os/scripts/auto-execute.sh && \
chmod +x agent-os/scripts/auto-execute.sh
```
...
```bash
mkdir -p agent-os/scripts
mv auto-execute.sh agent-os/scripts/
chmod +x agent-os/scripts/auto-execute.sh
```
...
```bash
sudo curl -fsSL https://raw.githubusercontent.com/michsindlinger/agent-os-extended/main/agent-os/scripts/auto-execute.sh \
...
./agent-os/scripts/auto-execute.sh

# Spezifischen Spec ausführen
./agent-os/scripts/auto-execute.sh 2026-01-13-feature-name
```
...
```
agent-os/specs/2026-01-13-feature-name/
...
```bash
SPECS_DIR="agent-os/specs"    # Pfad zu Specs
...
```bash
# Manuell prüfen
cat agent-os/specs/*/kanban-board.md | grep "Current Phase"
```
...
cat agent-os/specs/*/user-stories.md | head -20
...
nohup ./agent-os/scripts/auto-execute.sh > auto-execute.log 2>&1 &
...
watch -n 5 'cat agent-os/specs/*/kanban-board.md | grep -A5 "Resume Context"'
```
...
# agent-os/specs/2026-01-13-user-auth/ existiert mit user-stories.md

# 2. Script starten
./agent-os/scripts/auto-execute.sh 2026-01-13-user-auth
